...
DATA_ROOT="${DATA_ROOT:-$EXPERIMENT_ROOT/data_smoke}"
GPU_ID="${GPU_ID:-7}"
RUN_NAME="${RUN_NAME:-polis_mp_causal_smoke_$(date -u +%Y%m%d_%H%M%S)}"
...
exec "$VENV/bin/python" src/train.py \
  ...
  device.batch_size=1 device.eval_num_samples=1 device.num_workers=0 \
  runner.params.pretrained_model_path="$SOLARIS_ROOT/pretrained/mp_causal_60000.pt" \
  runner.params.total_steps=1 \
  runner.params.train_log_every_steps=1 \
  runner.params.test_log_every_steps=1000000 \
  runner.params.eval_every_steps=1000000 \
  runner.params.use_wandb=false \
  runner.params.checkpoint_config.params.save_interval_steps=1 \
  runner.params.checkpoint_config.params.max_to_keep=1 \
